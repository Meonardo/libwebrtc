...
void WebrtcVideoRendererD3D11Impl::OnFrame(webrtc::VideoFrameBuffer* buffer) {
  ...
  if (buffer->type() == webrtc::VideoFrameBuffer::Type::kNative) {
    ...
    auto scaled_buffer = I420Buffer::Create(window_width, window_height);
    auto i420_buffer = buffer->ToI420();
    scaled_buffer->ScaleFrom(*i420_buffer);

    RenderI420Frame_DX11(scaled_buffer.get());
  }
  return;
}
...
void WebrtcVideoRendererD3D11Impl::RenderNativeHandleFrame(
  ...
  RenderNV12DXGIMPO(buffer->width(), buffer->height());
}
...
void WebrtcVideoRendererD3D11Impl::RenderI420Frame_DX11(
  ...
  if (!InitSwapChain(buffer->width(), buffer->height(), false)) {
  ...
  if (!CreateStagingTexture(buffer->width(), buffer->height())) {
  ...
  RenderD3D11Texture(buffer->width(), buffer->height());
}
