...
int32_t CustomizedAudioCapturer::SetAudioDeviceSink(AudioDeviceSink* sink) {
  return -1;
}
  ...
  while (recording_) {
    ...
    if (last_call_record_millis_ == 0 ||
      ...
      audio_buffer_->SetRecordedBuffer(
          recording_buffer_.get(),
          recording_frames_in_10ms_);  // Buffer copied here
    ...
    need_sleep_ms_ =
        10 - cost_ms + need_sleep_ms_ - real_sleep_ms_ - loop_cost_ms;
    if (need_sleep_ms_ > 0) {
...
void CustomizedAudioCapturer::OnFrame(const uint8_t* data,
  ...
  // copy input audio frame data here
  audio_buffer_->SetRecordedBuffer(data, samples_per_channel);
  audio_buffer_->DeliverRecordedData();
}
